...
export default function CreatorSidebar({ pollId }) {
  ...
  return <>
    <header className="creator-header">
      <div className="creator-header-brand"><button className="menu-toggle" type="button" aria-label="Open navigation" aria-expanded={open} onClick={() => setOpen(true)}>☰</button><Link className="brand" to="/choice">LivePoll</Link></div>
      <div className="account-menu"><button className="account-trigger" type="button" aria-expanded={accountOpen} onClick={() => setAccountOpen((value) => !value)}><span className="avatar">{name.slice(0, 1).toUpperCase()}</span><span className="account-trigger-label">{name}</span><span aria-hidden="true">▾</span></button>{accountOpen && <div className="account-dropdown creator-account-dropdown"><strong>{name}</strong><small>{email}</small><Link to="/account" onClick={() => setAccountOpen(false)}>Account</Link><Link to="/settings" onClick={() => setAccountOpen(false)}>Settings</Link><button type="button" onClick={() => { setAccountOpen(false); logout(); }}>Sign Out</button></div>}</div>
    </header>
